...
class CurrencyConverter extends Component {
  ...
  //     this.setState({ info: nextState.info });
  //   }

  //   if (this.state.convertBase !== nextState.convertBase) {
  //     this.setState({ convertBase: nextState.convertBase });
  //   }
  // }

  fetchBaseCurrency = (event) => {
    ...
    fetch(`https://api.exchangeratesapi.io/latest?base=${cur}`, {
      ...
      .then((data) => {
        this.setState({
          ...
          convertBase: this.state.convertBase
        });
      });
  };

  fetchConvertCurrency = (event) => {
    let cur = event.target.value;
    fetch(`https://api.exchangeratesapi.io/latest?base=${cur}`, {
      "Access-Control-Allow-Origin": "*"
    })
      ...
      .then((data) => {
        console.log("fetchConvertCurrency data", data);
        this.setState({
          convertBase: data.base,
  ...
  convertInput = (event) => {
    ...
    const convertBase = this.state.convertBase;
    console.log("convertBase", convertBase);

    fx.base = baseCurrency;
    fx.rates = this.state.rates;
    let cvNum = parseFloat(this.state.baseValue / 100).toFixed(2);
    const res = fx(cvNum || 0)
      .from(baseCurrency)
      .to(convertBase);
    const convertedNumber = new Intl.NumberFormat("en-US", {
      style: "decimal"
    }).format(res);
    this.setState({
      ...
      <form>
        <div>
          <select
            id="selCurrency"
            name="base"
            onChange={this.fetchBaseCurrency}
          >
            {Object.keys(this.state.info).map((key, index) => {
              return (
                <option key={index} value={key}>
                  {this.state.info[key].name}
                </option>
              );
            })}
          </select>
          ...
          <select
            id="selCurrency"
            name="convert"
            onChange={this.fetchConvertCurrency}
          >
            {Object.keys(this.state.info).map((key, index) => {
              return (
                <option key={index} value={key}>
          ...
          converted currency:{" "}
          <input type="number" value={this.state.convertedValue} />
          <br></br>
          <button type="submit" onClick={this.convertInput}>
            <b>convert inputs</b>
          </button>
        </div>
      </form>
